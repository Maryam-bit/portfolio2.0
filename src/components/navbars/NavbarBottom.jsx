  ...
  const hovering = (id) => {
    ...
    for (let i = id; i >= 1; i--) {
      if (initialScale > 1) {
        ...
        if (id !== i) {
          eval(
            `navItem${i}`
          ).current.style.margin = `0 ${marginX}px 0 ${marginX}px`;
        }
    ...
    for (let i = id; i <= 7; i++) {
      if (initialRightScale > 1) {
        eval(
          `navItem${i}`
        ).current.style.transform = `scale(${initialRightScale})`;
        eval(`navItem${i}`).current.style.bottom = `${marginRightB}px`;
        eval(
          `navItem${i}`
        ).current.style.margin = `0 ${marginRightX}px 0 ${marginRightX}px`;
      }
    ...
    <div className={NavbarBottomStyles.navigation_bar}>
      <div className={`${NavbarBottomStyles.navbar_child} app-navbar`}>
        <Link to="/">
          <i
            ref={navItem1}
            onMouseOver={() => hovering(1)}
            onMouseOut={() => hoverOut(1)}
            className={`${NavbarBottomStyles.icon_3d} fa fa-house-damage icon_3d`}
          ></i>
        </Link>
        <Link to="projects">
          <i
            ref={navItem2}
            onMouseOver={() => hovering(2)}
            onMouseOut={() => hoverOut(2)}
            className={`${NavbarBottomStyles.icon_3d} fa fa-lightbulb icon_3d`}
          ></i>
        </Link>
        <Link to="services">
          <i
            ref={navItem3}
            onMouseOver={() => hovering(3)}
            onMouseOut={() => hoverOut(3)}
            className={`${NavbarBottomStyles.icon_3d} fas fa-hand-holding icon_3d`}
          ></i>
        </Link>
        <a
          href="https://github.com/Maryam-bit"
          target="_blank"
          rel="noreferrer"
        >
          <i
            ref={navItem4}
            onMouseOver={() => hovering(4)}
            onMouseOut={() => hoverOut(4)}
            className={`${NavbarBottomStyles.icon_3d} fab fa-github icon_3d`}
          ></i>
        </a>
        <a
          href="https://www.linkedin.com/in/maryam-noor-/"
          target="_blank"
          rel="noreferrer"
        >
          <i
            ref={navItem5}
            onMouseOver={() => hovering(5)}
            onMouseOut={() => hoverOut(5)}
            className={`${NavbarBottomStyles.icon_3d} fab fa-linkedin icon_3d`}
          ></i>
        </a>
        {darkTheme === false ? (
          <i
            ref={navItem6}
            onMouseOver={() => hovering(6)}
            onMouseOut={() => hoverOut(6)}
            className={`${NavbarBottomStyles.icon_3d} fas fa-sun icon_3d`}
            onClick={() => {
              setDarkTheme(true);
            }}
          ></i>
          ...
          <i
            ...
            onMouseOver={() => hovering(6)}
            onMouseOut={() => hoverOut(6)}
            className={`${NavbarBottomStyles.icon_3d} fad fa-moon-stars icon_3d`}
            onClick={() => {
              setDarkTheme(false);
            }}
          ></i>
        ...
        <i
          ref={navItem7}
          onMouseOver={() => hovering(7)}
          onMouseOut={() => hoverOut(7)}
          className={`${NavbarBottomStyles.icon_3d} fad fa-pencil icon_3d`}
        ></i>
      </div>
    </div>
  );
